...
<div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
    ...
    <div class="flex flex-wrap gap-3">
        <a href="exports/laporan_pdf.php?filter_jenis=<?= urlencode($filter_jenis) ?>&filter_bulan=<?= urlencode($filter_bulan) ?>" target="_blank" class="inline-flex items-center px-6 py-3 bg-red-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg shadow-red-500/20 active:scale-95">
            <i class="fas fa-file-pdf mr-2"></i> Ekspor PDF
        </a>
        <a href="exports/laporan_excel.php" target="_blank" class="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-green-700 transition-all shadow-lg shadow-green-500/20 active:scale-95">
...
<div class="bg-white p-6 rounded-[2rem] shadow-sm border border-secondary-100 mb-8">
    <form method="GET" action="dashboard_admin.php" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
            <label class="block text-[10px] font-black text-secondary-400 uppercase tracking-[0.2em] mb-2 ml-1">Jenis Transaksi</label>
            <select name="filter_jenis" class="w-full px-5 py-3 bg-secondary-50 border-0 focus:ring-2 focus:ring-primary-500 rounded-2xl text-secondary-900 font-medium">
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-secondary-50/50">
                    <th class="px-8 py-5 text-[10px] font-black text-secondary-400 uppercase tracking-widest border-b border-secondary-100">Waktu Transaksi</th>
                    <th class="px-6 py-5 text-[10px] font-black text-secondary-400 uppercase tracking-widest border-b border-secondary-100">Ringkasan Transaksi</th>
                    <th class="px-6 py-5 text-[10px] font-black text-secondary-400 uppercase tracking-widest border-b border-secondary-100">Arus</th>
                    <th class="px-8 py-5 text-[10px] font-black text-secondary-400 uppercase tracking-widest border-b border-secondary-100 text-right">Jumlah</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-secondary-50">
                <?php if ($laporan->num_rows > 0): ?>
                    ...
                    <tr class="hover:bg-primary-50/20 transition-all">
                        ...
                        <td class="px-6 py-5">
                            <p class="text-sm text-secondary-600 font-medium"><?= htmlspecialchars($row['keterangan']) ?></p>
                        </td>
                        <td class="px-6 py-5">
                            <span class="inline-flex px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest bg-<?= $color ?>-100 text-<?= $color ?>-600">
                                <?= $row['jenis'] ?>
                            </span>
                        </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="4" class="px-8 py-20 text-center">
                            ...
                            <p class="text-secondary-400 font-bold tracking-tight">Tidak ada data tersedia untuk laporan ini.</p>
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
